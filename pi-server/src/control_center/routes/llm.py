...
def amend_llm_configuration_route():
    ...
    err = amend_llm_configuration(
        endpoint=body.endpoint, model=body.model, api_key=body.api_key
    )
    if err is not None:
...
def get_available_llms_route():
    ...
    if not endpoint:
        return Response(
            response=Error(message="No endpoint provided!").as_json(),
...
def get_llm_endpoint_suggestions_route():
    ...
    return GetLLMEndpointSuggestionsResponse(
        endpoint_suggestions=endpoint_suggestions
    ).as_json()
