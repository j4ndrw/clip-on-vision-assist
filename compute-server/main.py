...
async def ai_stream(
    ...
    if x_llm_backend_endpoint is None:
        return Response(
            json.dumps({"error": "No LLM backend endpoint provided!"}),
            status_code=400,
    ...
    if x_llm_backend_api_key is None:
        return Response(
            json.dumps({"error": "No LLM backend API key provided!"}),
            status_code=400,
            media_type="application/json",
        )
    ...
    if x_llm is None:
        return Response(
            json.dumps({"error": "No LLM provided!"}),
            status_code=400,
            media_type="application/json",
        )
        ...
        ai_stream_state_machine(
            config=AIStreamStateMachineConfig(
                ai_system=AISystem(
                    ...
                    llm_client=LLMClient().use(
                        url=x_llm_backend_endpoint, api_key=x_llm_backend_api_key
                    ),
...
async def get_available_llms(
    ...
    try:
        models = (
            LLMClient()
            .use(url=x_llm_backend_endpoint, api_key=x_llm_backend_api_key)
            .get()
            .models.list()
        )
        ...
        return Response(
            json.dumps(models), status_code=200, media_type="application/json"
        )
    except openai.APIConnectionError:
        return Response(
            json.dumps({"error": "Could not reach the provided endpoint!"}),
            status_code=400,
            media_type="application/json",
        )
